...
if [ "$1" = "/gerrit-start.sh" ]; then
  ...
  if [ -z "$(ls -A "$GERRIT_SITE/git")" ]; then
    ...
    su-exec ${GERRIT_USER} java ${JAVA_OPTIONS} ${JAVA_MEM_OPTIONS} -jar "${GERRIT_WAR}" init --batch --install-all-plugins --no-auto-start -d "${GERRIT_SITE}"
  ...
  su-exec ${GERRIT_USER} cp -f ${GERRIT_HOME}/motd.jar ${GERRIT_HOME}/plugins/motd.jar
  su-exec ${GERRIT_USER} cp -f ${GERRIT_HOME}/uploadvalidator.jar ${GERRIT_HOME}/plugins/uploadvalidator.jar
  su-exec ${GERRIT_USER} cp -f ${GERRIT_HOME}/quota.jar ${GERRIT_HOME}/plugins/quota.jar
  su-exec ${GERRIT_USER} cp -f ${GERRIT_HOME}/avatars-gravatar.jar ${GERRIT_HOME}/plugins/avatars-gravatar.jar

  rm -f ${GERRIT_HOME}/events-log.jar
  rm -f ${GERRIT_HOME}/motd.jar
  rm -f ${GERRIT_HOME}/uploadvalidator.jar
  rm -f ${GERRIT_HOME}/quota.jar
  rm -f ${GERRIT_HOME}/avatars-gravatar.jar
